...
export default function Roadmap() {
  ...
  if (loading || !user) {
    return <div className="min-h-screen bg-background flex justify-center items-center font-bold text-white uppercase tracking-widest">Loading CodeArena...</div>;
  }
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <header className="mb-8 border-b-2 border-[#333] pb-6 flex items-center gap-4">
            <div className="bg-brand-yellow neo-border p-3 text-black">
              <Map size={32} />
            </div>
            <div>
              <h1 className="text-4xl font-black uppercase tracking-widest text-white drop-shadow-[2px_2px_0px_#FACC15]">
                {roadmapData.title}
              </h1>
          ...
          <div className="space-y-10">
            {roadmapData.levels.map((level) => (
              <div key={level.id} className="relative pl-10 border-l-4 border-brand-yellow">
                ...
                <h2 className="text-3xl font-black text-white uppercase tracking-widest drop-shadow-[2px_2px_0px_#EF4444] mb-6">
                  {level.levelName}
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {level.skills.map((skill) => (
                    <div key={skill.id} className="neo-card bg-[#1f1f1f] text-white border-brand-blue p-6 hover:-translate-y-2 hover:shadow-[4px_4px_0px_#3B82F6] transition-all">
                      ...
                      <ul className="space-y-3">
                        {skill.topics.map((topic, index) => (
                          <li key={index} className="flex items-start gap-2 font-medium text-white/90">
