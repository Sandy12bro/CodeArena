  ...
  return (
    <div className="neo-card neo-card-white p-6 h-full flex flex-col hover:-translate-y-2 hover:shadow-[8px_8px_0px_#000000]">
      <div className="flex items-center gap-3 mb-6 pb-4 border-b-2 border-black">
        ...
        <h2 className="text-2xl font-black uppercase tracking-widest text-black">Top Performers</h2>
      </div>
        ...
        {LEADERBOARD_DATA.map((user) => (
          <div 
            ...
            className={`flex items-center justify-between p-3 neo-border rounded-md ${
              user.isUser ? "bg-brand-yellow shadow-[2px_2px_0px_#000000]" : "bg-white"
            }`}
          >
            <div className="flex items-center gap-3">
              <span className={`font-black text-lg w-6 flex justify-center ${user.rank === 1 ? 'text-brand-red text-xl' : 'text-black'}`}>
                {user.rank}
              </span>
              <div className="w-8 h-8 rounded-full neo-border bg-gray-200 flex justify-center items-center font-bold text-xs">
                {user.name.charAt(0)}
              </div>
              <div>
                <p className="font-bold text-black text-sm">{user.name}</p>
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{user.badge}</p>
              </div>
            </div>
