...
export default function ActivityFeed() {
  const { activities, searchQuery } = useDashboard();
    ...
    <div className="neo-card p-6 h-full min-h-[400px]">
      ...
      <div className="space-y-4">
        {filteredActivities.length === 0 ? (
          <p className="opacity-50 text-sm font-black uppercase tracking-widest text-center py-10">
            {searchQuery ? `No activities matching "${searchQuery}"` : "No signals detected yet..."}
          </p>
        ) : (
          filteredActivities.map((act, i) => {
            const Icon = IconMap[act.icon] || Star;
            const isLatest = i === 0 && !searchQuery; // Only highlight latest if not searching
            return (
