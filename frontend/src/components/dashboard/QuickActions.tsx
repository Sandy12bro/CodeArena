        ...
        {filtered.length > 0 ? (
          filtered.map((action, i) => (
            <div 
              ...
              className={`neo-card ${action.tint} ${action.accent} p-6 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all duration-200 hover:-translate-y-2 ${action.shadow} group rounded-md`}
            >
              <div className="p-3 bg-card border-2 border-border rounded-md group-hover:rotate-6 transition-transform shadow-[3px_3px_0px_#000]">
